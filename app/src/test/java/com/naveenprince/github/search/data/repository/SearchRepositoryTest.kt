package com.naveenprince.github.search.data.repository
    ...
    fun testSearchUsers_Success() = runTest {
        ...
        result.collect { apiResponseStatus ->
            when (apiResponseStatus) {
                is ResponseStatus.Success -> Assert.assertEquals(
        ...
        result.collect { apiResponseStatus ->
            when (apiResponseStatus) {
                ...
                is ResponseStatus.Error -> {
                    Assert.assertEquals(errorCode, apiResponseStatus.statusCode)
                    Assert.assertEquals(errorMessage, apiResponseStatus.message)
                }
            }
        }
        Mockito.verify(remoteDataSource).searchUsers(query)
